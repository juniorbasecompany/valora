...
- Modelo [`Log`](backend/src/valora_backend/model/log.py): `account_id`, `tenant_id`, `table_name`, `action_type`, `row` (JSONB), `moment_utc` (default no servidor).
...
2. **Listener `before_flush` na classe `Session`**  
   ...
   - **Ações:**  
     ...
     - `deleted` → `action_type = 'D'`, `row_payload = {}` conforme comentário do modelo.  
   - **Uma linha `Log` por entidade afetada** por flush (simples); opcional futuro: agrupar ou filtrar colunas.

3. **Serialização para JSONB**  
   Função dedicada (ex.: `entity_to_audit_dict(instance) -> dict`) usando `sqlalchemy.inspect`, valores escalares, conversão de `datetime` para ISO string, `Decimal` para string ou float conforme política, relações **não** expandir por defeito (só colunas mapeadas na tabela) para evitar explosão de tamanho e ciclos.
...
- **Unitário:** listener com `Session` em memória (SQLite de teste): verificar que um `add`/`delete` gera `Log` quando contextvars preenchidos; que `Log` não gera segundo log; que tabela fora da lista não gera entrada.  
...
- [ ] Testes cobrindo com e sem contexto, e exclusão de `log`.
